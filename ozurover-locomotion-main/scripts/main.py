...
def calculate_velocity_targets(rotation, speed):
    # Rover dimensions
    rover_width = 0.64  # in meters

    # When no rotation, both wheels should have the same speed
    if rotation == 0:
        return [speed, speed]

    # Calculate the turning radii for inner and outer wheels
    if speed != 0:
        rot_radius = abs(speed / rotation)
        ...
        # For in-place rotation (tank turn), one wheel moves forward and the other backward
        inner_velocity = -rotation * (rover_width / 2)
        outer_velocity = rotation * (rover_width / 2)

    if rotation < 0:
        # Turning left: left wheel is inner, right wheel is outer
        velocity_left_group = inner_velocity
        velocity_right_group = outer_velocity
    else:
